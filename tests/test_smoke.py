...
def test_all_sources_down_still_completes(tmp_path, capsys):
    ...
    assert flags == 14  # 7 FRED + CFTC + Yahoo + EIA + EDGAR + GDELT + NASS + IMF
    run_detail = conn.execute("SELECT detail FROM journal"
                              " WHERE event_type = 'run'").fetchone()[0]
    assert "14 fetch failures" in run_detail
    assert conn.execute("SELECT COUNT(*) FROM series").fetchone()[0] == 15
    conn.close()
...
def test_partial_run_derives_net_liquidity(tmp_path, capsys):
    ...
    run_detail = conn.execute("SELECT detail FROM journal"
                              " WHERE event_type = 'run'").fetchone()[0]
    assert "7 fetch failures" in run_detail  # all but FRED down (CFTC/Yahoo/
    # EIA/EDGAR/GDELT/IMF — cloud + NASS keyless here, so those legs fail)
    conn.close()
